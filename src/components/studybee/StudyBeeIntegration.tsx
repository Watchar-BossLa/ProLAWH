...
import { StudyBeeErrorBoundary } from './StudyBeeErrorBoundary';
import { StudyBeeOfflineMode } from './StudyBeeOfflineMode';
import { 
  ExternalLink, 
  Clock, 
  BookOpen, 
...
export function StudyBeeIntegration() {
  ...
  // Show offline mode if there's a network error
  if (error && error.includes('connection')) {
    return (
      <StudyBeeErrorBoundary>
        <StudyBeeOfflineMode 
          onRetry={refreshData}
          isRetrying={loading}
        />
  ...
  if (loading && !progress && sessions.length === 0) {
    return (
      <Card>
        <CardContent className="flex items-center justify-center py-8">
          <div className="flex items-center gap-2">
            <Loader2 className="h-4 w-4 animate-spin" />
            <span>Loading Study Bee integration...</span>
          </div>
        </CardContent>
      </Card>
    );
