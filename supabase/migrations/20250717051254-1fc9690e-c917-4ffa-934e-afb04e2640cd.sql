-- Phase 1: Critical Security Hardening - Fix All Database Security Issues (Final)
-- This migration addresses all 25 security linter issues identified

-- 1. Enable RLS on public tables that don't have it (excluding views)
...
-- 2. Create missing tables first (if they don't exist)
...
-- 3. Drop existing policies that might conflict, then recreate them

-- Content templates policies
DROP POLICY IF EXISTS "Authenticated users can view templates" ON public.content_templates;
DROP POLICY IF EXISTS "Only admins can manage templates" ON public.content_templates;
...
DROP POLICY IF EXISTS "Only admins can manage achievements" ON public.environmental_achievements;

CREATE POLICY "Anyone can view achievements" ON public.environmental_achievements
  FOR SELECT USING (true);

-- Incorporation status policies
DROP POLICY IF EXISTS "Project creators can view incorporation status" ON public.incorporation_status;
DROP POLICY IF EXISTS "Project creators can update incorporation status" ON public.incorporation_status;
...
DROP POLICY IF EXISTS "Project creators can manage entity documents" ON public.business_entity_documents;
...
DROP POLICY IF EXISTS "Only admins can manage staking contracts" ON public.staking_contracts;

CREATE POLICY "Anyone can view active staking contracts" ON public.staking_contracts
  FOR SELECT USING (active = true);

-- 4. Fix all database functions to have secure search_path (SECURITY DEFINER with proper search_path)
...
-- 5. Create performance and security indexes
...
CREATE INDEX IF NOT EXISTS idx_staking_contracts_active ON public.staking_contracts(active);
