-- Phase 1: Critical Security Hardening - Fix All Database Security Issues (Corrected)
...
-- Note: active_stakes and green_skill_index are views, not tables
...
-- 3. Create user_roles table if it doesn't exist (needed for functions)
...
-- 4. Create security function for role checking
...
-- Now add admin policies that depend on the function
CREATE POLICY "Only admins can manage templates" ON public.content_templates
  FOR ALL USING (public.has_user_role(auth.uid(), 'admin'));
...
-- 5. Create user_sessions table if it doesn't exist
...
-- 6. Create security_audit_logs table if it doesn't exist
...
CREATE POLICY "Only admins can view security audit logs" ON public.security_audit_logs
  FOR SELECT USING (public.has_user_role(auth.uid(), 'admin'));

CREATE POLICY "System can insert security audit logs" ON public.security_audit_logs
  FOR INSERT WITH CHECK (true);

-- 7. Create staking_contracts table if it doesn't exist
...
-- 8. Fix all database functions to have secure search_path
