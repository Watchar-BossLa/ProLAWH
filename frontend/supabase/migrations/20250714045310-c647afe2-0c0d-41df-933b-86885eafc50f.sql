-- Add typing indicators table (only if it doesn't exist)
...
-- Enable RLS for typing indicators (only if not enabled)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_tables 
    WHERE tablename = 'chat_typing_indicators' AND rowsecurity = true
  ) THEN
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
    ...
    ALTER TABLE chat_messages ADD COLUMN status text DEFAULT 'sent';
  END IF;
END $$;